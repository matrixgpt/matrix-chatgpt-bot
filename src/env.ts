...
export const {
  ...
  CHATGPT_API_MODEL: {
    schema: z.string().default(""),
    description:
      "The model for the ChatGPT-API to use. Keep in mind that these models will charge your OpenAI account depending on their pricing.",
  },
  CHATGPT_PROMPT_PREFIX: {
    schema: z
      .string()
  ...
  CHATGPT_REVERSE_PROXY: {
    schema: z.string().default(""),
    description:
      "Change the api url to use another (OpenAI-compatible) API endpoint",
  },
  CHATGPT_TEMPERATURE: {
    schema: z.number().default(0.8),
    description: "Set the temperature for the model",
  },
  CHATGPT_MAX_CONTEXT_TOKENS: {
    schema: z.number().default(4097),
    description:
      "Davinci models have a max context length of 4097 tokens, but you may need to change this for other models.",
  },
  CHATGPT_MAX_PROMPT_TOKENS: {
    schema: z.number().default(3097),
    description:
